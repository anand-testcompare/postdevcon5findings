...
The `promptSdkVersion` function is not an LLM prompt. It is the interactive CLI step that decides which OSDK template line to scaffold. In practice, that means `create-app` is carrying compatibility policy for generated apps, not just copying files.

The meaningful additions in the 2.7 generation were: bootstrapping without an existing OSDK, adding `@osdk/foundry` to templates, and making the default SDK choice the latest compatible template rather than the earliest one. The 2.7.5 patch itself is mostly upkeep.
